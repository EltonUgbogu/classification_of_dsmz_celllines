...
Graph analyses used the node set specified for the corresponding graph. Patient-referenced per-lineage graphs and resolved neighbour graphs used lineage-specific graph nodes; for the final retinoblastoma support-threshold and resolved graphs, RBL\_15 and RBL\_20 replicate profiles were collapsed to biological cell-line groups by arithmetic mean aggregation of tumour-neighbourhood consensus vectors before graph construction. The pan-cancer cell-line network retained the 167 analysed RNA-seq profiles as profile-level nodes. Consequently, retinoblastoma counts can differ across profile-level ranking analyses, biological-group graph analyses, biological-group ranking analyses, and pan-cancer profile-level networks.
...
structure before and after source harmonisation and for the curated 257-gene
feature-set projection \citep{McInnes2020}. No quantitative inference was made
from UMAP coordinates.
...
analysed cell-line RNA-seq profiles defined in the dataset table. This
cross-lineage visualisation retained RBL replicate libraries as profile-level
nodes and was separate from the final biological-group retinoblastoma
neighbourhood graph.
...
detection used \texttt{igraph::cluster\_louvain()} with edge weights,
random seed 1, and no explicit Louvain resolution parameter for the reported partition;
...
\]
Mean Precision@k at the displayed absolute grid
\(k \in \{1,5,10,25,50,100\}\) was calculated across
...
Patient consensus-network components refer to precomputed tumour graph
component assignments for patient tumour samples from the prior patient-cohort
tumour similarity graph analysis; operationally, these were fixed
graph-clustering component labels, not communities recalculated during the
cell-line-centred ranking step.
To quantify how high-ranking tumour samples were distributed across these
components, the top 50 ranked tumour samples for each biological cell-line
group were joined to their consensus component identifiers. For each component
$C_k$, the component size $n_k = |C_k|$ was defined
...
were used where specified. Pan-cancer Louvain and Leiden community detection
were run with random seed 1, whereas the representation-specific
