...
suppressPackageStartupMessages({
  ...
  library(UpSetR)        # Creates UpSet plots for visualising set intersections
                         ...
                         # Note: UpSetR uses grid graphics internally but doesn't
                         # require explicit loading of grid/gridExtra packages
})

# =============================================================================
# HARDCODED PATHS (for standalone execution)
# =============================================================================
# These paths are used when the script is run directly (not via Snakemake).
# In a pipeline context, these would be provided via command-line arguments
# or a configuration file.

vst_rds <- "/home/ugbogu/data/BRCA/tcga_vst_brca_subtype_batch_corrected.rds"
outdir <- "/work/ugbogu/expr_transformer/fs/results"
...
run_unsupervised_feature_selection <- function(
  ...
  # variance because it's in the same units as the data.
  #
  ...
  save_upset_with_legend <- function(sets, outfile_png, title = "") {
    # ---------------------------------------------------------------------------
    # SIMPLIFIED: UpSet plot only (no grid manipulation, no legend panel)
    # ---------------------------------------------------------------------------
    # This version avoids all grid/base graphics mixing issues by simply
    # rendering the UpSet plot directly. UpSetR handles its own grid graphics
    # internally, and we don't interfere with it.
    #
    # The legend information is documented in the script comments and can be
    # referenced separately if needed. This approach is the most reliable on
    ...
    # Debug: Print set information for verification
    # This helps verify that all sets are being passed to UpSetR
    message("[DEBUG] Sets in UpSet plot: ", paste(names(sets), collapse = ", "))
    message("[DEBUG] Set sizes: ", paste(vapply(sets, length, integer(1)), collapse = ", "))
    ...
    # Use cairo type if available (better rendering on HPC); fallback to regular PNG
    tryCatch({
      png(outfile_png, width = 2200, height = 1400, res = 180, type = "cairo")
    }, error = function(e) {
      png(outfile_png, width = 2200, height = 1400, res = 180)
    })
    
    # Render UpSet plot directly - UpSetR handles all grid graphics internally
    # CRITICAL: print() is required for UpSetR to render on headless systems
    # nsets = length(sets) forces display of ALL sets (not just top 5)
    # keep.order = TRUE preserves the input order of sets
    print(
      UpSetR::upset(
        upset_input,
        nsets = length(sets),        # Show ALL sets (not just top 5)
  ...
  # centre=FALSE because the data is already centred
  ...
  final_top3000_spearman <- names(sort(avg_corr, decreasing = TRUE))[1:top_n_method]
  ...
  final_top3000_mx <- names(sort(mx_score, decreasing = TRUE))[1:top_n_method]
  ...
  if (!quiet) message("[DEBUG] Intersection length: ", length(genes))
  ...
  if (length(genes) < 100) {
    ...
    warning("[WARNING] Expected ~", length(all_candidates), " genes")
  ...
  if (length(genes) == 0) {
    ...
    if (!quiet) message("[WARNING] Creating empty scores_df")
  ...
  if (length(sets_3000_use) >= 2) {
    ...
    if (!quiet) message("[INFO] UpSet plot (top3000) saved")
  } else {
    warning("[WARNING] Insufficient sets for UpSet plot (top3000)")
  }
  ...
  if (length(sets_500_use) >= 2) {
    ...
    if (!quiet) message("[INFO] UpSet plot (top500) saved")
  } else {
    warning("[WARNING] Insufficient sets for UpSet plot (top500)")
  }
  ...
  if (!quiet) {
    message("[DEBUG] Writing: ", joint_tsv, " (", nrow(scores_df), " rows)")
    message("[DEBUG] Writing: ", genes_file, " (", length(final_top500_mx), " genes)")
  }
  
  readr::write_tsv(scores_df, joint_tsv)
  writeLines(final_top500_mx, genes_file)
  
  # Individual method top500 lists
...
fs_subdir <- outdir
cfg <- list(feature_selection = list(top_n_method = 3000, final_top = 500))
