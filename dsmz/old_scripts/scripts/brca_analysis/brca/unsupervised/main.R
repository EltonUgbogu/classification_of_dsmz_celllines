...
pca_pre_BC <- make_pca_plot(merged_exp_data$Xc_raw, merged_exp_data$batch, "PCA Pre-Batch Correction", file.path(config$outdir, "pca_pre_BC.pdf"), scale = TRUE)
pca_post_BC <- make_pca_plot(vst_batch_corrected_counts, coldata$dataset, "PCA Post-Batch Correction", file.path(condif$outdir, "pca_post_BC.pdf"), scale = FALSE)
umap_pre_BC <- make_umap(merged_exp_data$Xc_raw, "UMAP Pre-Batch Correction", file.path(config$outdir, "umap_pre_BC.pdf"), batch_labels = merged_exp_data$batch, scale = TRUE)
umap_post_BC <- make_umap(vst_batch_corrected_counts, "UMAP Post-Batch Correction", file.path(config$outdir, "umap_post_BC.pdf"), batch_labels = coldata$dataset, scale = FALSE)
saveRDS(dsmz_vst_batch_corrected, file.path(config$outdir, "dsmz_vst_batch_corrected.rds"))
saveRDS(tcga_vst_batch_corrected, file.path(config$outdir, "tcga_vst_batch_corrected.rds"))
