...
common_genes <- function(tcga_counts, dsmz_counts, min_shared=NULL) {
  cat(sprintf("[INFO] DSMZ count matrix: %d genes x %d samples\n", nrow(dsmz_counts), ncol(dsmz_counts)))  # Print dimensions of DSMZ count matrix M                                                    # Return DSMZ count matrix
  cat(sprintf("[INFO] TCGA count matrix: %d genes x %d samples\n", nrow(tcga_counts), ncol(tcga_counts)))  # Print dimensions of TCGA count matrix
