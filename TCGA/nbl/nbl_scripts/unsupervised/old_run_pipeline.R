...
#olddata directory would be changed to newdata directory later
tcga_se_rds   <- "/home/chu25/data/tcga/olddata/dALL_TCGA_STAR_Counts_SummarizedExperiment_filtered.rds"
dsmz_rds      <- "/home/chu25/data/dsmz/DSMZ_count_gene.rds"
dsmz_meta_csv <- "/home/chu25/data/dsmz/DSMZ_metadata.csv"
outdir <- "/home/chu25/dsmz/results/nbl_vst_dual3"
...
if (!is.null(dsmz_organ_filter)) {
  ...
  dsmz_nbl_counts <- dsmz_counts[, keep_ids, drop = FALSE]
}

log_dims("DSMZ (after organ subset)", dsmz_nbl_counts)

if (!is.null(dsmz_organ_filter) && ncol(dsmz_nbl_counts) == 0L) {
...
# ------------------- 1. Harmonize samples & subset to TCGA-BRCA -------------------
  ...
  # BRCA-only filter
  #tcga_is_nbl <- as.character(colData_tcga[, "project_id"]) == "TCGA-BRCA"
  #if (!all(tcga_is_nbl, na.rm = TRUE)) {
  #  keep_names <- rownames(colData_tcga)[tcga_is_nbl]
...
if (!is.null(tcga_project_filter)) {
  ...
  tcga_nbl_counts <- tcga_counts[, keep_tcga, drop = FALSE]
}

log_dims("TCGA (after project subset)", tcga_nbl_counts)

if (!is.null(tcga_project_filter) && ncol(tcga_nbl_counts) == 0L) {
  stop(sprintf("[FATAL] TCGA project filter %s left 0 samples.", paste(tcga_project_filter, collapse=", ")))
}

## ---------- 4. Intersect genes -------------------
common <- intersect(rownames(tcga_nbl_counts), rownames(dsmz_nbl_counts))
cat(sprintf("[INFO] Shared genes: %d\n", length(common)))
if (length(common) < 1000) cat("[WARN] Few shared genes; check Ensembl IDs\n")

## ---------- 5. Subset to common genes -------------------
tcga_nbl_counts <- tcga_nbl_counts[common, , drop = FALSE]
dsmz_nbl_counts <- dsmz_nbl_counts[common, , drop = FALSE]

log_dims("TCGA (post-intersect)", tcga_nbl_counts)
log_dims("DSMZ (post-intersect)", dsmz_nbl_counts)

## ---------- 6. Merge Breast Cancer Counts from TCGA and DSMZ datasets -------------------
Xc_raw <- cbind(tcga_nbl_counts, dsmz_nbl_counts)
...
write_dims_line(con, "DSMZ (after organ subset)", dsmz_nbl_counts)
write_dims_line(con, "TCGA (after project subset)", tcga_nbl_counts)
writeLines(sprintf("Shared genes\t%d", length(common)), con)
write_dims_line(con, "TCGA (post-intersect)", tcga_nbl_counts)
write_dims_line(con, "DSMZ (post-intersect)", dsmz_nbl_counts)
