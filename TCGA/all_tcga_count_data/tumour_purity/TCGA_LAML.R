...
subset_se_to_tcga_project <- function(se, project = "TCGA-LAML") {
  ...
  if (length(hit_cols) == 0) {
    stop("[ERROR] Could not find a project/cohort column in colData() to filter ", project, ".")
  }
...
run_tidyestimate_purity_analysis <- function(
  ...
  output_dir = "/home/chu25/TCGA/all_tcga_count_data/tumour_purity/results/TCGA_LAML",
  gtf_path = "/home/chu25/data/GTF/Homo_sapiens.GRCh38.107.gtf",
  assay_name = NULL,
  save_updated = TRUE,
  purity_threshold = 0.7,
  apply_purity_filter = TRUE,
  focus_project = "TCGA-LAML"
) {
  ...
  # Subset to one TCGA project if requested
  ...
  # Filter to ESTIMATE signature genes (manual filter like RBL script to avoid filter_common_genes bugs)
  ...
  if ("tumourPurity" %in% colnames(estimate_scores)) {
    ...
    # Define tag once for use in plot titles
    tag <- if (!is.null(focus_project) && nzchar(focus_project)) focus_project else "TCGA_ALL"
    
    # RBL-style palette for purity groups
    ...
    # A: Purity vs StromalScore (RBL styling)
    p1 <- ggplot(plot_data, aes(x = StromalScore, y = tumourPurity, colour = PurityGroup)) +
      ...
      ggpubr::stat_cor(method = "pearson",
                       ...
                       label.x.npc = "left", colour = "black") +
      labs(
        title = paste0(tag, ": Tumour Purity vs Stromal Score"),
        x     = "Stromal Score",
        y     = "Tumour Purity"
      ) +
    ...
    # B: Purity vs ImmuneScore (RBL styling)
    p2 <- ggplot(plot_data, aes(x = ImmuneScore, y = tumourPurity, colour = PurityGroup)) +
      ...
      ggpubr::stat_cor(method = "pearson",
                       ...
                       label.x.npc = "left", colour = "black") +
      labs(
        title = paste0(tag, ": Tumour Purity vs Immune Score"),
        x     = "Immune Score",
        y     = "Tumour Purity"
      ) +
      theme_minimal(base_size = 13) +
    ...
    # C: Purity distribution (single cohort; use RBL palette fill, keep RBL threshold styling)
    ...
    p3 <- ggplot(plot_data, aes(x = tumourPurity)) +
      ...
      labs(
        title = paste0(tag, ": Distribution of Tumour Purity"),
        x     = "Tumour Purity",
        y     = "Number of Samples"
      ) +
      theme_minimal(base_size = 13) +
