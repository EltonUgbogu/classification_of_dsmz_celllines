#!/usr/bin/env python3
"""
rbl_cohort_aggregator.py
RBL (Retinoblastoma) COHORT PROCESSING PIPELINE
Merges STAR count files → two matrices:
    1. final_rbl_count_matrix.parquet        → samples × Ensembl_ID
    2. final_rbl_count_matrix_hgnc.parquet   → samples × HGNC_Symbol
Outputs:
    • final_rbl_count_matrix.parquet
    • final_rbl_count_matrix_hgnc.parquet
    • final_rbl_sample_metadata.csv
    • final_rbl_gene_annotation.csv
    • final_rbl_ensembl_to_hgnc.tsv
    • final_rbl_gene_list_ensembl.txt
    • final_rbl_gene_list_hgnc.txt
"""
import pandas as pd
...
# CONFIGURATION
# ==============================
ROOT_DIR = Path("/home/chu25/data/rbl")
COHORTS = ["GSE111168", "GSE196420", "GSE268136"]   # ← correct RBL cohorts

# INPUT MAPS (same as before – Ensembl ↔ HGNC is universal)
...
# OUTPUTS – now RBL-specific
MATRIX_ENS       = ROOT_DIR / "final_rbl_count_matrix.parquet"
MATRIX_HGNC      = ROOT_DIR / "final_rbl_count_matrix_hgnc.parquet"
META_FILE        = ROOT_DIR / "final_rbl_sample_metadata.csv"
GENE_ANNOT_CSV   = ROOT_DIR / "final_rbl_gene_annotation.csv"
ENS_TO_HGNC_TSV  = ROOT_DIR / "final_rbl_ensembl_to_hgnc.tsv"
GENE_LIST_ENS    = ROOT_DIR / "final_rbl_gene_list_ensembl.txt"
GENE_LIST_HGNC   = ROOT_DIR / "final_rbl_gene_list_hgnc.txt"
...
def main():
    ...
    print("Starting RBL cohort aggregation...\n")

    for cohort in COHORTS:
        cohort_path = ROOT_DIR / cohort / "results" / "star" / "valid_count"
