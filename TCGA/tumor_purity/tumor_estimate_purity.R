# =============================================================================
# TIDYESTIMATE PURITY ANALYSIS PIPELINE
# =============================================================================
# Description:
#   - Converts Ensembl gene IDs to HGNC symbols using a GTF annotation file.
#   - Filters expression data to ESTIMATE signature genes.
#   - Computes ESTIMATE scores (Stromal, Immune, ESTIMATE) using tidyestimate.
#   - Derives TumorPurity from ESTIMATEScore if not provided.
#   - Optionally filters samples by tumor purity threshold (>= 0.6).
#   - Saves scores to CSV, updates SummarizedExperiment colData, and generates
#     diagnostic correlation plots.
#
# Dependencies:
#   tidyestimate, SummarizedExperiment, dplyr, ggplot2, ggpubr, data.table, tibble
# =============================================================================
...
# -- Helper: Map Ensembl IDs to HGNC symbols -----------------------------------
...
standardize_estimate_scores <- function(scores_df) {
  ...
  # Derive TumorPurity using Yoshihara et al. formula if missing
  if (!"TumorPurity" %in% colnames(scores_df) && "ESTIMATEScore" %in% colnames(scores_df)) {
    scores_df$TumorPurity <- cos(0.6049872018 + 0.0001467884 * scores_df$ESTIMATEScore)
    scores_df$TumorPurity <- pmin(1, pmax(0, scores_df$TumorPurity))  # Clamp to [0,1]
    message("[INFO] Derived TumorPurity from ESTIMATEScore.")
  }
  
  # Final validation
  required_cols <- c("StromalScore", "ImmuneScore", "ESTIMATEScore", "TumorPurity")
...
# -- Helper: Filter samples by tumor purity threshold -------------------------
filter_samples_by_purity <- function(se_object, purity_threshold = 0.6) {
  message("[INFO] Applying tumor purity filtering (threshold >= ", purity_threshold, ")...")
  ...
  message("[INFO] Tumor purity summary:")
...
run_tidyestimate_purity_analysis <- function(
  ...
  stopifnot("[ERROR] Input file not found: ", file.exists(tcga_se_path))
  tcga_se <- readRDS(tcga_se_path)
  
  # Extract expression matrix
  expr_matrix <- if (is.null(assay_name)) assay(tcga_se) else assay(tcga_se, assay_name)
  ...
  # Update SummarizedExperiment with TumorPurity
  result <- list(
  ...
  if (save_updated && "TumorPurity" %in% colnames(estimate_scores)) {
    common_samples <- intersect(rownames(estimate_scores), colnames(tcga_se))
    colData(tcga_se)$purity <- NA_real_
    colData(tcga_se)[common_samples, "purity"] <- estimate_scores[common_samples, "TumorPurity"]
    
    saveRDS(tcga_se, tcga_se_updated_path)
    message(sprintf("[INFO] Updated colData with TumorPurity (%d/%d samples). Saved to: %s",
  ...
  if (apply_purity_filter && "TumorPurity" %in% colnames(estimate_scores)) {
    ...
    filtered_counts <- assay(tcga_se_filtered)
  ...
  if ("TumorPurity" %in% colnames(estimate_scores)) {
    ...
    p1 <- ggplot(plot_data, aes(x = StromalScore, y = TumorPurity)) +
      geom_point(alpha = 0.6, size = 1.2) +
      geom_smooth(method = "lm", se = FALSE, color = "blue") +
      stat_cor(method = "spearman", label.y = 0.1) +
      stat_cor(method = "pearson", label.y = 0.05) +
      theme_bw(base_size = 12) +
      labs(title = "Tumor Purity vs Stromal Score", x = "Stromal Score", y = "Tumor Purity")
    
    p2 <- ggplot(plot_data, aes(x = ImmuneScore, y = TumorPurity)) +
      geom_point(alpha = 0.6, size = 1.2) +
      geom_smooth(method = "lm", se = FALSE, color = "blue") +
      stat_cor(method = "spearman", label.y = 0.1) +
      stat_cor(method = "pearson", label.y = 0.05) +
      theme_bw(base_size = 12) +
      labs(title = "Tumor Purity vs Immune Score", x = "Immune Score", y = "Tumor Purity")
    
    p3 <- ggplot(plot_data, aes(x = TumorPurity)) +
      ...
      theme_bw(base_size = 12) +
      labs(title = "Distribution of Tumor Purity", x = "Tumor Purity", y = "Sample Count")
    
    # Save combined plot
    plot_file <- file.path(output_dir, "purity_correlation_diagnostics.pdf")
    combined_plot <- ggarrange(p1, p2, p3, ncol = 2, nrow = 2, labels = "AUTO")
