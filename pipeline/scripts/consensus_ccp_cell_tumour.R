...
build_expr_mat <- function(feature, cfg, kind) {
  ...
  # --------------------------------------------------------------------------
  # For feature sets defined by specific selection methods (Variance, MAD,
  # Spearman, PCA loadings, Entropy, etc.), expression submatrices are
  # loaded from pre-computed files generated by the feature selection step.
  
  unsup_root <- cfg$paths$unsup_root
  if (is.null(unsup_root) || !nzchar(unsup_root)) {
    stop_with("Missing paths.unsup_root in config (required for feature set: ", feature, ")")
  }
  
  # Resolve the unsupervised analysis root to an absolute path.
  is_abs <- grepl("^/", unsup_root) || grepl("^[A-Za-z]:", unsup_root)
  if (!is_abs) {
    config_dir <- dirname(normalizePath(opt$config))
    pipeline_root <- dirname(config_dir)
    unsup_root <- file.path(pipeline_root, unsup_root)
  }
  
  # Construct paths to feature set files.
  feature_expr_path <- file.path(unsup_root, "feature_selection_unsupervised", 
                                 "featuresets", feature, "expr_submatrix.rds")
  feature_map_path <- file.path(unsup_root, "tumour_neighbourhoods_input", 
                                paste0("cell_line_to_original_sample_id_", feature, ".rds"))
  
  if (!file.exists(feature_expr_path)) {
    stop_with("Missing feature set expression matrix: ", feature_expr_path)
  }
  
  if (!file.exists(feature_map_path)) {
    stop_with("Missing feature set mapping file: ", feature_map_path)
  }
  
  # Load the expression matrix and sample mapping.
  expr_mat <- readRDS(feature_expr_path)
  mapping <- readRDS(feature_map_path)
  
  # Standardise matrix orientation (samples in rows, genes in columns).
  expr_mat <- ensure_samples_in_rows(expr_mat)
  
  # Extract sample identifiers.
  sample_ids <- rownames(expr_mat)
  if (is.null(sample_ids) || length(sample_ids) == 0) {
    stop_with("Expression matrix has no rownames (expected sample IDs)")
  }
  
  # Determine sample origin using the mapping file.
  # The mapping contains names that correspond to DSMZ cell line IDs.
  dsmz_ids <- names(mapping)
  cell_samples <- intersect(sample_ids, dsmz_ids)
  tumour_samples <- setdiff(sample_ids, cell_samples)
  
  if (length(cell_samples) == 0) {
    stop_with("No DSMZ/cell-line samples found")
  }
  
  if (length(tumour_samples) == 0) {
    stop_with("No tumour samples found")
  }
  
  # Create dataset annotation vector.
  dataset <- ifelse(sample_ids %in% dsmz_ids, "DSMZ", "TCGA")
  names(dataset) <- sample_ids
  
  # Apply sample scope filtering.
  if (is_cell_tumour) {
    # Use all samples (no filtering needed)
  } else if (is_cell_only) {
    expr_mat <- expr_mat[cell_samples, , drop = FALSE]
    dataset <- dataset[cell_samples]
  } else if (is_tumour_only) {
    expr_mat <- expr_mat[tumour_samples, , drop = FALSE]
    dataset <- dataset[tumour_samples]
  } else {
    stop_with("Cannot determine scope from kind: ", kind)
  }
  
  # Transpose to genes x samples (standard orientation for downstream processing).
  expr_mat <- t(expr_mat)
