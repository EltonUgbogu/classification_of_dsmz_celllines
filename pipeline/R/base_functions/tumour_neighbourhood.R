...
# For each cell line, the algorithm identifies the most similar TCGA tumour
# samples based on expression distance, enabling biological interpretation of
# cell line-tumour relationships.
#
...
# clustering solution. The function identifies the nearest TCGA tumour samples
# for each cell line based on expression distance.
#
# PARAMETERS:
...
#   dataset:    Named vector indicating sample origin ("DSMZ" or "TCGA").
...
#     - n_tcga:         Number of TCGA tumours in the dataset
# ------------------------------------------------------------------------------
...
compute_tumour_neighbourhoods <- function(
  ...
  # error message to guide the user toward the correct input format.
  ...
  # ----------------------------------------------------------------------------
  # Samples are partitioned into DSMZ cell lines (query set) and TCGA tumours
  # (target set). The neighbourhood algorithm computes distances from each
  # cell line to all candidate tumours.
  ...
  for (cell_line in dsmz_ids) {
    ...
    # Handle missing cluster assignments gracefully.
    ...
    # Fallback: if the cluster contains too few tumours, expand to all TCGA.
  ...
  # using dplyr::bind_rows(). This function efficiently concatenates data
  # frames with potentially different columns.
  ...
  #   - Primary results (neighbourhoods and long_df)
  #   - Metadata (method_id, distance metric)
  #   - Parameters (for reproducibility documentation)
  #   - Summary statistics (sample counts)
  
  list(
    neighbourhoods = neighbourhoods,   # List: cell_line → vector of top TCGA tumour IDs
    long_df       = long_df,           # Tidy data frame for consensus voting
    method_id     = method_id,
    distance      = distance,          # Distance metric used
    params        = list(
      top_frac  = top_frac,
      top_n_min = top_n_min,
      top_n_max = top_n_max
