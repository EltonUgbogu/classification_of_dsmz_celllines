...
snakemake --configfile config/config.yaml --config pipeline_profile=rbl --list-rules 2>&1 | tee /tmp/rbl_rules.txt
grep -i "dsmz\|similarity_graph" /tmp/rbl_rules.txt || echo "RULE NOT IN LIST"
```
...
snakemake --configfile config/config.yaml --config pipeline_profile=rbl -n -p all 2>&1 | tee /tmp/rbl_dag.txt
grep -i "dsmz_dsmz_similarity_graph\|DSMZ_DSMZ_graph_edges" /tmp/rbl_dag.txt || echo "NOT IN DAG"
```
