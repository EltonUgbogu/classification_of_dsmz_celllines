...
if (is.null(directions) || length(directions) == 0) directions <- default_dirs
...
# ---- 5) Simple comparison plot ----
p <- ggplot(dir_summary, aes(x = direction, y = frac_ge_thr)) +
  geom_col(width = 0.7) +
  scale_y_continuous(labels = percent_format(accuracy = 1), limits = c(0, 1)) +
  labs(
    title = "p_consensus strength by direction",
    subtitle = sprintf("Strong consensus defined as p_consensus >= %.2f", thr),
    x = "Direction",
...
ggsave(file.path(out_dir, "Fig_p_consensus_direction_comparison.pdf"), p, width = 8, height = 4.8)
