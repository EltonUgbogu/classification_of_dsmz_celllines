...
#   6) Save filtered matrices and generate cohort-level plots.
#
# Exam emphasis:
#   The script enforces identifier consistency, makes sample filtering traceable,
#   and provides quantitative checks showing how purity relates to stromal
...
#   - generates checks to justify the filtering choice
run_tidyestimate_with_barplot <- function(
  ...
  output_dir       = Sys.getenv(
                       "OUTPUT_DIR",
                       file.path(repo_root, "data", "nbl", "results", "tumour_purity_analysis")
                     ),
  ...
  tryCatch({
    ...
    # Write the exact retained sample order and cohort labels for the downstream
    # batch-correction rule. Keeping this manifest beside the filtered count
    # object prevents downstream code from reconstructing sample provenance
    # from names or from the unfiltered metadata.
    retained_manifest <- data.frame(
      sample = kept_samples,
      cohort = extract_project(kept_samples, sample_to_cohort),
      stringsAsFactors = FALSE
    )
    retained_manifest_out <- file.path(
      output_dir,
      sprintf("nbl_retained_samples_purity%.1f.tsv", purity_threshold)
    )
    utils::write.table(
      retained_manifest,
      file = retained_manifest_out,
      sep = "\t",
      quote = FALSE,
      row.names = FALSE
    )
    message("[INFO] Retained-sample manifest saved to: ", retained_manifest_out)
    ...
    diag_pdf <- file.path(output_dir, "nbl_purity_distribution_and_scores.pdf")
...
# Purity threshold: 0.7 is the fixed threshold for this project.
# This value was chosen to exclude samples with substantial non-tumour signal
# (stromal/immune contamination) while retaining a sufficient number of samples
# for downstream analysis.
#
# Note for future research:
#   The threshold is a parameter and can be adjusted to explore the effect of
#   more stringent or more lenient filtering on downstream results. To experiment,
#   pass a different value via the PURITY_THRESHOLD environment variable or
#   the purity_threshold argument. The purity_distribution_and_scores.pdf generated by
...
#   OUTPUT_DIR="${REPO_ROOT}/preprocessing_and_quality_control/nbl/results/tumour_purity_analysis" \
