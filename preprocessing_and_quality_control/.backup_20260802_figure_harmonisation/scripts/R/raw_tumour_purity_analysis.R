...
## Empty panel with a recessive grid, hairline axes and a left-aligned title.
## `x_axis = FALSE` suppresses the numeric x scale for categorical panels.
figure_panel_frame <- function(xlim, ylim, xlab, ylab, main = NULL, subtitle = NULL,
                               x_axis = TRUE) {
  graphics::plot.new()
  ...
  if (x_axis) {
    graphics::abline(v = x_ticks, col = FIGURE_RULE, lwd = 0.6)
  }
  graphics::abline(h = y_ticks, col = FIGURE_RULE, lwd = 0.6)
  if (x_axis) {
    graphics::axis(
  ...
  graphics::title(xlab = xlab, line = 2.5, col.lab = FIGURE_INK)
  graphics::title(ylab = ylab, line = 3.1, col.lab = FIGURE_INK)
  if (!is.null(main)) {
    graphics::mtext(
      main, side = 3, line = 1.4, adj = 0, font = 2,
      cex = 1.1 * graphics::par("cex"), col = FIGURE_INK
    )
  }
  if (!is.null(subtitle)) {
    graphics::mtext(
      subtitle, side = 3, line = 0.3, adj = 0,
      cex = 0.82 * graphics::par("cex"), col = FIGURE_INK_MUTED
    )
  }
  invisible(NULL)
}

figure_threshold_line <- function(value, orientation) {
  if (identical(orientation, "v")) {
    graphics::abline(v = value, col = FIGURE_INK, lwd = 1.8, lty = 2)
  } else {
    graphics::abline(h = value, col = FIGURE_INK, lwd = 1.8, lty = 2)
  }
  invisible(NULL)
}
...
grDevices::pdf(snakemake@output[["purity_diagnostics"]], width = 12, height = 8.6, pointsize = 11)
graphics::layout(
...
graphics::legend(
  "center",
  legend = c(
    sprintf("Retained, purity >= %.2f (n = %s)", threshold, figure_count(retained_total)),
    sprintf("Excluded, purity < %.2f (n = %s)", threshold, figure_count(excluded_total)),
...
graphics::rect(
  ...
  as.vector(bar_midpoints) + bar_half_width,
  bar_values,
  col = rep(c(FIGURE_INK_MUTED, FIGURE_RETAINED), times = length(cohort_levels)),
  border = "#ffffff",
  lwd = 1.2
)
graphics::text(
...
graphics::legend(
  "topright",
  legend = c("Before purity filter", "After purity filter"),
  fill = c(FIGURE_INK_MUTED, FIGURE_RETAINED),
  border = "#ffffff",
  bty = "n",
  cex = 1.0,
  text.col = FIGURE_INK
