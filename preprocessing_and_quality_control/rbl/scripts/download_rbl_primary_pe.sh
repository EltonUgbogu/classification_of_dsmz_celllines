#!/bin/bash
# =============================================================================
# download_rbl_primary_pe.sh
#
# Downloads, extracts, and compresses paired-end SRA data for a retinoblastoma
# cohort. For each SRR accession in a plain-text list, the script executes a
...
#   <DATASET>/removed_rbl_srr_id_primary_tumour_pe.txt
#
...
#
# Usage:
#   sbatch download_rbl_primary_pe.sh [DATASET|all]
#   DATASET defaults to GSE268136_primary_tumours if not supplied.
#   Use "all" to process every directory matching GSE*_primary_tumours under ROOT
#   sequentially within the same Slurm job (one allocation, one job ID). Each
#   cohort runs in its own subshell with stdout/stderr and per-tool logs under
#   that cohort's logs/ directory.
#
# Slurm #SBATCH --output/--error are set to /dev/null because paths cannot depend
# on DATASET; after startup the script redirects into "$BASE/logs/".
# =============================================================================
...
run_one_dataset() {
    ...
    local REMOVED_FILE="${BASE}/removed_rbl_srr_id_primary_tumour_pe.txt"
    local PREFETCH_LOG_DIR="${LOG_DIR}/prefetch"
    local FQDUMP_LOG_DIR="${LOG_DIR}/fasterq_dump"
    local PIGZ_LOG_DIR="${LOG_DIR}/pigz"

    # Validate before creating anything under BASE (avoid mkdir for bogus DATASET names).
    if [[ ! -d "$BASE" ]]; then
        mkdir -p "${ROOT}/logs"
        echo "ERROR: Dataset directory does not exist: $BASE" >>"${ROOT}/logs/download_rbl_primary_pe_${SLURM_JOB_ID:-manual}.err"
        return 2
    fi
...
if [[ "$ARG" == "all" ]]; then
    shopt -s nullglob
    cohort_dirs=("${ROOT}"/GSE*_primary_tumours)
    if [[ ${#cohort_dirs[@]} -eq 0 ]]; then
        mkdir -p "${ROOT}/logs"
        echo "ERROR: no directories matching ${ROOT}/GSE*_primary_tumours" >>"${ROOT}/logs/download_rbl_primary_pe_${SLURM_JOB_ID:-manual}.err"
        exit 2
    fi

    overall_status=0
    for d in "${cohort_dirs[@]}"; do
        if ! run_one_dataset "$(basename "$d")"; then
            overall_status=1
        fi
    done
    exit "$overall_status"
